...
for i in range(7):
	if coor[0][i] > 0:
		...
		new1[0].append(np.amin(np.array(data)[:,i]))
		new2[0].append(np.amax(np.array(data)[:,i]))
ter = []
err = []
ent = []
...
plt.figure()
plt.plot(err)
plt.savefig('test_err_'+str(batch)+'_'+str(l_rate)+'_661_norm.png')
plt.figure()
plt.plot(ent)
plt.savefig('learning_curve_'+str(batch)+'_'+str(l_rate)+'_661_norm.png')
plt.figure()
plt.plot(ter)
plt.savefig('train_err_'+str(batch)+'_'+str(l_rate)+'_661_norm.png')
